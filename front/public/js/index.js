...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const userInfoLink = document.querySelector('.nav__section__user a[href^="/users/"]');
    if (userInfoLink) {
        userInfoLink.addEventListener("click", function (e) {
            // userData.id 값 확인을 위한 요소 존재 여부 체크
            const userIdElement = document.querySelector('.header__user p[style="display: none"]');
            if (!userIdElement || !userIdElement.textContent) {
                e.preventDefault(); // 기본 동작 중지
                alert("회원 정보를 수정하기 위해선 로그인을 먼저 해주세요");
